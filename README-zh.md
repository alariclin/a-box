...
本项目仅用于**网络架构测试、网络安全研究、系统运维自动化以及在完全授权环境中的合法隐私保护**。

1. **法律合规**：请勿将本项目用于任何违反所在国家或地区法律法规、服务条款或网络使用政策的活动。

2. **授权边界**：请仅在您拥有所有权、管理权或明确授权的服务器、网络和系统中部署、测试或运行本项目。

3. **用户责任**：用户需自行承担因误用、违规使用、配置错误或操作不当导致的全部法律、运维、安全和合规后果。

4. **技术意图**：本项目涉及的路由、加密、代理、流量控制和诊断功能，旨在提高授权环境中的传输安全性、可用性和运维效率。严禁用于非法攻击、未授权访问、规避审计、破坏网络基础设施或协助任何违法行为。

5. **无担保声明**：本项目按“现状”提供，不承诺适用于任何特定用途，也不保证在所有网络、系统或司法辖区中可用、合规或无风险。

6. **接受条款**：下载、复制、部署或运行本项目，即表示您已阅读、理解并接受上述条款。
...
### 脚本提示没有交互式 TTY。
从交互式终端运行。如果管道运行失败，先下载脚本，再执行 `sudo bash A-Box.sh`。

### 部署因端口占用失败。
脚本会检查占用所选端口的非 A-Box 进程。请手动释放端口，或选择其他端口。

### 预检查会阻止重装已部署的协议栈吗？
不会。协议部署前的轻量预检查不会因为 A-Box 托管服务已占用端口而失败。实际部署流程会先停止托管服务，再写入新协议栈。

### 备份和诊断包包含什么？
备份包含 A-Box 配置、服务文件、脚本、部分防火墙/cron 状态和相关运行元数据。诊断包会对敏感值进行脱敏。

### Hysteria 2 为什么要求填写上行/下行带宽？
Hysteria 2 会把配置的 `up` 和 `down` 作为带宽控制与拥塞控制参数。请按 VPS 线路容量填写。

### ACME 证书申请失败。
HTTP-01 模式下，确认 `80/TCP` 可达且未被占用。Cloudflare DNS-01 模式下，确认 API Token 对目标 zone 具有 DNS 编辑权限。

### 应该如何选择 SNI？
使用工具箱 `3` 或 `4` 运行 SNI 优选，再用工具箱 `10` 查看保存结果。优先选择 TLS 1.3、SAN 命中、ALPN 正常，并与 VPS ASN/拓扑关系合理的候选。
